...
def serve(port: int, fmt: str = "both", key: str = DEFAULT_KEY, fail_first: int = 0, host: str = "127.0.0.1") -> ThreadingHTTPServer:
    handler = type("H", (Handler,), {"fmt": fmt, "key": key, "fail_first": fail_first, "seen": []})
...
def main() -> None:
    ...
    a = ap.parse_args()
    srv = serve(a.port, a.format, a.key, a.fail_first, a.host)
    print(f"mock gateway ({a.format}) on http://{a.host}:{a.port}/v1  key={a.key}")
    try:
        srv.serve_forever()
